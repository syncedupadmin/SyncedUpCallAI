...
/** Type matches your analysis JSON */
...
function toLocal(dt?: string | null) {
  if (!dt) return "";
  const d = new Date(dt);
  return isNaN(d.valueOf()) ? dt : d.toLocaleString();
}
const minutes = (s: number) => `${Math.floor(s / 60)}m ${s % 60}s`;

/** Tailwind class maps (no dynamic class strings that get purged in prod) */
...
function Progress({ value }: { value: number }) {
  ...
  return (
    <div className="w-full h-2 rounded bg-slate-200">
      <div className="h-2 rounded bg-slate-800" style={{ width: `${safe}%` }} />
    </div>
  );
}
...
function Chip({ text }: { text: string }) {
  return <span className="inline-flex items-center rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-800">{text}</span>;
}
...
/** Reason → badge tone */
const reasonTone: Record<string, keyof typeof badgeMap> = {
  pricing: "amber",
  spouse_approval: "blue",
  bank_decline: "rose",
  benefits_confusion: "violet",
  trust_scam_fear: "rose",
};
    ...
    <div className="bg-white p-6">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900">Call Analysis</h1>
            <p className="text-sm text-slate-600">
              Model {d.model} • v{d.version} • Confidence {Math.round(d.confidence * 100)}%
            </p>
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 p-4">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <div className="text-sm text-slate-600">
              Callback window: {d.best_callback_window ? `${toLocal(d.best_callback_window.local_start)} → ${toLocal(d.best_callback_window.local_end)}` : "—"}
            </div>
          </div>
        </div>
            ...
            <Section title="Scores">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  <Meter label="QA Score" value={d.qa_score} />
                  <Meter label="Script Adherence" value={d.script_adherence} />
                </div>
                <div className="space-y-3">
                  <div className="text-sm text-slate-600">Sentiment (agent/customer)</div>
                  <div className="flex gap-2 text-sm">
                    <Chip text={`agent ${d.sentiment_agent.toFixed(2)}`} />
                    <Chip text={`customer ${d.sentiment_customer.toFixed(2)}`} />
                    <Chip text={`ASR ${d.asr_quality}`} />
                  </div>
                </div>
              </div>
                ...
                {d.key_quotes?.map((q, i) => (
                  <li key={i} className="rounded-lg border border-slate-200 p-3">
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                      <span>{q.ts}</span>
                      <span>•</span>
                      <span className="uppercase">{q.speaker}</span>
                    </div>
          ...
          <div className="space-y-4">
            <Section title="Outcome & Risks">
              <div className="flex flex-wrap gap-2">
                {d.risk_flags?.length ? d.risk_flags.map((r, i) => <Chip key={i} text={r} />) : <Chip text="no risk flags" />}
              </div>
              {!!d.compliance_flags?.length && (
                <div className="flex flex-wrap gap-2">
                  {d.compliance_flags.map((r, i) => <Chip key={i} text={`compliance: ${r}`} />)}
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2">
                {d.actions.map((a, i) => (
                  <button
                    ...
                    onClick={() => alert(`${a} (wire to CRM/GHL)`)}
                  >
                    {a.replaceAll("_", " ")}
                  </button>
                ))}
