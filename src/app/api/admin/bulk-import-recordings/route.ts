...
export async function POST(req: NextRequest) {
  ...
  setTimeout(async () => {
    try {
      ...
      // Note: The actual Convoso API might need different parameters for date range
      // This is based on the existing endpoint structure
      ...
      console.log('[BULK IMPORT] Fetching from:', url);
      ...
      if (!response.ok) {
        ...
        sendProgress({
          status: 'error',
          message: `Convoso API error: ${response.status}`,
          current: 0,
          total: 0
        });
        controller.close();
